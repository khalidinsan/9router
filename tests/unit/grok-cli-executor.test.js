...
describe("GrokCliExecutor", () => {
  ...
  it("supportsGrokCliReasoningEffort is only true for grok-4.5 family", () => {
    ...
    expect(supportsGrokCliReasoningEffort("grok-code-fast-1")).toBe(false);
  ...
  it("transformRequest strips effort for composer but keeps summary (upstream protocol)", () => {
    ...
    // Composer rejects effort but still accepts summary / encrypted continuity
  ...
  it("transformRequest strips effort for official grok-composer-2.5-fast", () => {
    const out = executor.transformRequest(
