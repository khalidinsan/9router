...
describe("fetchAntigravityWeeklyQuota", () => {
  ...
  it("sends correct headers and body", async () => {
    ...
    // Fork: CLI fingerprint + MITM anti-loop flag on all quota traffic
    // (the IDE UA is only for web-search).
    const { platform, arch } = await import("os");
    const forkUA = `antigravity/cli/1.1.27 (aidev_client; os_type=${platform()}; arch=${arch()}; cl=976543523; auth_method=consumer)`;
...
describe("weekly quota isolation from existing quota", () => {
    ...
    proxyAwareFetch.mockImplementation(async (url) => {
      ...
      if (url.includes(":fetchAvailableModels")) {
        ...
        return {
          ...
          status: 200,
          json: async () => body,
          // models fetch parses text() itself (retry on truncated cold-start body)
  ...
  it("reconciles weekly quota to 0% when all paid-tier family models are exhausted", async () => {
    ...
    // Per-model quota is unknown (no remainingFraction) — recorded as null so
    // routing fails open to the group bucket instead of blocking the model.
    expect(result.quotas["gemini-3.8-flash-high"].remainingPercentage).toBeNull();
