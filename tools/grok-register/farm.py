...
def fill_code_and_submit(email, dev_token, timeout=60):
    # Poll IMAP for OTP via email_imap, then fill the code.
...
def convert_and_push_grok_cli(result: dict) -> None:
    ...
    # UI (9router Add Account) / pool env override config.json
    oauth_mode = str(
        os.environ.get("GROK_OAUTH_MODE")
        or gcli.get("oauth_mode")
        or "pkce"
    ).strip().lower()
    referrer = str(
        os.environ.get("GROK_OAUTH_REFERRER")
        or gcli.get("oauth_referrer")
        or "grok-build"
    ).strip() or "grok-build"
    try:
        settle_s = float(
            os.environ.get("GROK_POST_SIGNUP_SETTLE_S")
            or gcli.get("post_signup_settle_sec")
            or 12
        )
    except (TypeError, ValueError):
        settle_s = 12.0
    inject_policy = str(
        os.environ.get("GROK_INJECT_POLICY")
        or gcli.get("inject_policy")
        or "usable"
    ).strip().lower()
    ...
    if os.environ.get("GROK_JWT_REJECT_BOT_FLAG", "").strip().lower() in (
        "1", "true", "yes", "on",
    ):
        reject_bot = True
    ...
    if enforce_ref is None:
        enforce_ref = True
    probe_model = str(
        os.environ.get("GROK_CHAT_PROBE_MODEL")
        or gcli.get("chat_probe_model")
        or gcli.get("smoke_model")
        or "grok-4.5"
    ).strip()
...
def main():
    ...
    WORKER_ID = wid
    # This worker's share (from --count); pool env set by pool.py
