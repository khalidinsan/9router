"""
Push Grok Build OAuth tokens into 9router provider `grok-cli`.

Modes (env NINEROUTER_IMPORT_MODE):
  direct  — emit @@GROK_CLI_IMPORT@@ marker for parent 9router process
            (createProviderConnection in-process). Default when launched
            from Add Account automation.
  http    — POST /api/providers (standalone CLI / external farm).

Auth for HTTP (same pattern as Gsuiteto9router/bot.js):
...
Bot-flag / usable gate:
  JWT bot_flag_source alone is NOT a hard ban (many flagged tokens still chat OK).
  Farm pipeline prefers chat_usable probe before calling this module.
  When the access token is bot-flagged, we still smoke-test cli-chat-proxy
  unless smoke is disabled; import only if the probe succeeds.
"""
...
def push_build_tokens_to_9router(
    ...
    *,
    ...
    """
    Create grok-cli connection via 9router (direct marker and/or HTTP).

    Prefer direct mode when embedded (NINEROUTER_IMPORT_MODE=direct).
    Always emits @@GROK_CLI_IMPORT@@ so the parent runner can import.
    ...
    Bot-flag policy (default):
      - no bot_flag_source → import
      - bot_flag_source set → smoke-test model on cli-chat-proxy;
    ...
    if mode in ("direct", "inprocess", "local", "none", "skip-http"):
        print(
            ...
            f"(no HTTP) name={name_s} email={email_s}",
            flush=True,
        )
        out: Dict[str, Any] = {
            "id": "",
