...
export async function handleAntigravityQuotaError(connectionId, status, model, accessToken, providerSpecificData) {
  ...
  // Group-aware: prefers the per-model bucket, else the governing group's weekly
  // bucket. A null remainingPercentage means group-governed with no group data
  // yet — the strike breaker below counts that as "unknown", not as exhausted.
  ...
  if (!quota || quota.remainingPercentage == null || quota.remainingPercentage > 0) {
    const key = `${connectionId}|${model}`;
    const now = Date.now();
    ...
    if (count >= STRIKE_THRESHOLD) {
      ...
      const reading = quota?.remainingPercentage != null ? `${Math.round(quota.remainingPercentage)}%` : "unknown";
